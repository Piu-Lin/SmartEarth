  ...
  dataSource.load("/asd.czml").then(() => {
    ...
    if (Geoeye1entity && ISSentity && Molniya1 && Geoeye1ToISS && AGIToISS) {
      const updateDescription = () => {
        ...
        const Molniya1Position = Molniya1.position.getValue(clock.currentTime);
        const AGIPosition = AGI.position.getValue(clock.currentTime);

        if (AGIPosition) {
          const agiPosition = Cesium.Cartographic.fromCartesian(AGIPosition);
          const longitude = Cesium.Math.toDegrees(
            agiPosition.longitude
          ).toFixed(2);
          const latitude = Cesium.Math.toDegrees(agiPosition.latitude).toFixed(
            2
          );
        ...
        if (ISSentityPosition) {
          const cartographicPosition =
            Cesium.Cartographic.fromCartesian(ISSentityPosition);
          const longitude = Cesium.Math.toDegrees(
            cartographicPosition.longitude
          ).toFixed(6);
          const latitude = Cesium.Math.toDegrees(
        ...
        if (Molniya1Position) {
          const cartographicPosition =
            Cesium.Cartographic.fromCartesian(Molniya1Position);
          const longitude = Cesium.Math.toDegrees(
            cartographicPosition.longitude
          ).toFixed(6);
          const latitude = Cesium.Math.toDegrees(
