...
export default class BaseHandler {
  ...
  initLeftClick() {
    ...
    handler.setInputAction((e) => {
      ...
      if (viewer.scene.mode !== Cesium.SceneMode.MORPHING) {
        ...
        if (pickedObject) {
          ...
          if (Cesium.defined(pickedObject) && pickedObject.id) { // 如果定义了对象，则返回true，否则返回false。
            const entity = pickedObject.id;
            // 调用父页面方法
            window.parent.postMessage({ id: entity.id, name: entity.name, Type: entity.Type }, "*");
    ...
    // 左键按下事件
    handler.setInputAction(throttle((e) => {
      if (!this.destroyAroundPoint) {
        this.aroundPoint();
      }
    ...
    }, 100), Cesium.ScreenSpaceEventType.MIDDLE_DOWN);
    // 左键双击事件
    handler.setInputAction((e) => {
      if (!this.destroyAroundPoint) {
        this.aroundPoint();
      ...
      if (pickedObject) {
        ...
        if (pickedObject.id.Type !== 'village' && pickedObject.id.Type !== 'grid') return;
        // 移除原先的高亮面实体
        this.removeHighlightPolygonLayer();
        if (pickedObject.id.Type === 'village') {
          ...
          if(pickedObject.id.x && pickedObject.id.y) {
            this.flyTo({x:parseFloat(pickedObject.id.x),y:parseFloat(pickedObject.id.y),z:200});
          } else {
          ...
          viewer.dataSources._dataSources.forEach((dataSource) => {
            ...
            if (dataSource.name === 'minigrid') {
              dataSource.entities.values.forEach((entity) => {
                if (entity.block === pickedObject.id.name && entity.village === pickedObject.id.village) {
                  entity.show = true;
                }
              });
            }
    ...
    handler.setInputAction(
        throttle((e) => {
              if(this.pickedEntity){
                this.pickedEntity.polygon.material = Cesium.Color.fromCssColorString("#1e2d43").withAlpha(0.8);
                this.pickedEntity.polyline.material = Cesium.Color.fromCssColorString("#ffffff");
                this.pickedEntity = null;
              }
              const pickedObject = viewer.scene.pick(e.endPosition);
              if (pickedObject) {
                if (Cesium.defined(pickedObject) && pickedObject.id && pickedObject.id.Type) {
                  if(pickedObject.id.Type === 'grid' || pickedObject.id.Type === 'minigrid'){
                    pickedObject.id.polygon.material = Cesium.Color.fromCssColorString("#213e57").withAlpha(0.8);
                    pickedObject.id.polyline.material = Cesium.Color.fromCssColorString("#15fcff");
  ...
  // 监听相机高度
  cameraHeight(){
    this.viewer.camera.changed.addEventListener(throttle(()=>{
  ...
  pickedFBX(pickedObject) {
    if (pickedObject.primitive.name === 'nanxun-fbx-sn') {
      ...
      this.viewer.scene.primitives._primitives.forEach(primitive => {
        if(primitive.name === 'nanxun-fbx-sw' || primitive.name === 'nanxun-fbx-dx') {
          primitive.show = true
        }
