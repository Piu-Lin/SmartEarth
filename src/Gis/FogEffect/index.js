...
export default class FogEffect {
    ...
    // 雾粒子效果
    createParticleForFog(type, flag) {
        if (type === "Fog" && flag === true) {
            this.init();
        } else if (flag === false) {
            this.destroy();
        }
    ...
    init() {
        this.visibility = Cesium.defaultValue( 0.2);
        this.color = Cesium.defaultValue( new Cesium.Color(0.8, 0.8, 0.8, 0.3), new Cesium.Color(0.8, 0.8, 0.8, 0.3));
