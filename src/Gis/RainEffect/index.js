export default class RainEffect {
    ...
    // 雨粒子效果
    createParticleForRain(type, flag) {
        if (type === "Rain" && flag === true) {
            this.init();
        } else if (flag === false) {
            this.destroy();
        }
    ...
    init() {
        this.tiltAngle = Cesium.defaultValue( -.6);
        this.rainSize = Cesium.defaultValue( 0.6);
        this.rainSpeed = Cesium.defaultValue( 120.0);
