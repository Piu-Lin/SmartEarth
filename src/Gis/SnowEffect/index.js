export default class SnowEffect {
    ...
    // 雪粒子效果
    createParticleFroSnow(type, flag) {
        if (type === "Snow" && flag === true) {
            this.init();
        } else if (flag === false) {
            this.destroy();
        }
    ...
    init() {
        this.snowSize = Cesium.defaultValue(0.02);
        this.snowSpeed = Cesium.defaultValue(60.0);
    ...
    snow() {
        return "uniform sampler2D colorTexture;\n\
            ...
            {\n\
                ...
                uv*=scale;vec2 s=floor(uv),f=fract(uv),p;float k=3.,monomer;\n\
                p=.5+.35*sin(11.*fract(sin((s+p+scale)*mat2(7,3,6,5))*5.))-f;monomer=length(p);k=min(monomer,k);\n\
